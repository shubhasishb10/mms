...
public interface RetailRepository extends JpaRepository<Retail, Long> {
    ...
    @Query(value = "select * from retail group by retailDate order by retailDate asc", nativeQuery = true)
    Page<Retail> findAllByOrderByRetailDateAsc(Pageable pageable);
